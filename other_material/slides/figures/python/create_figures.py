...
plot_folder = Path("slides/figures/generated/")
...
def _add_query_flavor_legend(fig: plt.Figure, query_flavors: List[str]) -> None:
    ...
    handles = [
        Line2D(
            ...
            markersize=5, label=QUERY_FLAVOR_LABELS[flavor],
        )
...
def _add_team_size_legend(
    ...
    for d_value in d_values:
        ...
        swatches.add_artist(Line2D([4], [4], linestyle="", marker="o", markersize=5,
                                   color=style.palette[f"balanced_selective,{d_value}"]))
        swatches.add_artist(Line2D([13], [4], linestyle="", marker="^", markersize=5,
...
def _draw_direction_arrows(ax: plt.Axes, df: pd.DataFrame) -> None:
    ...
    for _, flavor_data in df.groupby("QueryDescr"):
        ...
        ax.plot(means[VOLUME_X], means[VOLUME_Y], color=base_colors[3], linewidth=1.2, zorder=2)
        start, end = means.iloc[-2], means.iloc[-1]
        ax.annotate(
            "",
            xy=(end[VOLUME_X], end[VOLUME_Y]),
            xytext=(start[VOLUME_X], start[VOLUME_Y]),
            arrowprops={"arrowstyle": "->", "color": base_colors[3], "lw": 1.2},
            zorder=4,
        )


def plot_incremental_volume_vs_overhead(
...
def create_incremental_volume_sequences(
    source_path: Path = data_folder / "composition_variation_experiment.parquet",
    target_path: Path = plot_folder,
) -> None:
    """Create one four-frame comparison sequence at `b=10`.

    Each frame cumulatively reveals one team size for both query flavors.
    """
    ...
    for query_flavor in ("balanced_selective", "diverse"):
        ...
        flavor_data[query_flavor] = selected_flavor_data

    for frame_number, final_d in enumerate(d_values, start=1):
        frame_data = sequence_data.loc[sequence_data["d"] <= final_d]
        plot_incremental_volume_vs_overhead(
            frame_data,
            target_path / f"vol_vs_overhead_comparison_{frame_number}.pdf",
            style=style,
            d_values=sorted(frame_data["d"].unique()),
...
def plot_incremental_runtime(
    ...
    *,
    ...
    metric_legend_handles = [
        Line2D([], [], color="black", linestyle="", marker=flavor_markers[flavor], markersize=6,
...
def create_incremental_runtime_storage_sequence(
    source_path: Path = data_folder / "composition_variation_experiment.parquet",
    target_path: Path = plot_folder,
    only_averages: bool = True
) -> None:
    """Create two raw-runtime frames and one shared-storage line plot."""
    results = pd.read_parquet(source_path)
    target_path.mkdir(parents=True, exist_ok=True)
    sequence_data = _select_experiments(
        results,
    ...
    for frame_number, query_flavors in enumerate((
        ["balanced_selective"],
        ["balanced_selective", "diverse"],
    ), start=1):
        plot_incremental_runtime(
            sequence_data,
            target_path / f"runtime_comparison_{frame_number}.pdf",
            style=style,
...
def plot_selectivity_runtime(
    ...
    *,
    ...
    symbol_handles = [Line2D([0], [0], color="black", marker=mk, linestyle="None") for mk in marker_map.values()]
    symbol_labels = [
...
def create_dimensional_scaling_sequence(
    data_path: Path = data_folder / "SDSS_dimensional_scaling.parquet",
    target_path: Path = plot_folder / "dimensional_scaling_SDSS.pdf",
) -> None:
    """Create the full-data dimensional-scaling figure for the backup slide."""
    _plot_sdss_scaling(_aggregate_sdss_scaling_data(_load_sdss_scaling_data(data_path)), target_path)


def create_figure_8(
    data_path: Path = data_folder / "SDSS_dimensional_scaling.parquet",
    target_path: Path = plot_folder / "figure_8_dimensional_scaling_SDSS.pdf",
) -> None:
    """Rebuild Figure 8 from valid SDSS dimensional-scaling measurements only."""
    data = _load_sdss_scaling_data(data_path)
    data_without_va = data.loc[data["Index"] != SDSS_VA_KEY[1]].copy()
    if data_without_va.empty:
        raise ValueError("No non-VA SDSS dimensional-scaling measurements found.")
    if data_without_va["Index"].eq(SDSS_VA_KEY[1]).any():
        raise ValueError("VA-file measurements must not be included in Figure 8.")

    target_path.parent.mkdir(parents=True, exist_ok=True)
    plot_selectivity_runtime(
        _aggregate_sdss_scaling_data(data_without_va),
        line_by=["s", "Index"],
        symbol_by=("s",),
        color_by=("Index",),
        order_by="D",
        ...
        figure_size=(4.0, 2.2),
        markersize=5,
        colors=FIGURE_8_COLORS,
        color_legend_title="Index",
        symbol_legend_title="Predicate sel.",
        target_path=target_path,
    )


def create_incremental_dimensional_scaling_sequence(
    data_path: Path = data_folder / "SDSS_dimensional_scaling.parquet",
    target_path: Path = plot_folder,
) -> None:
    """Create a VA-first sequence with cumulative automatic-team selectivity frames."""
    data = _load_sdss_scaling_data(data_path)
    ...
    for frame_number, final_selectivity in enumerate(selectivities, start=1):
        frame_data = automatic_means.loc[automatic_means["s"] <= final_selectivity]
        _plot_sdss_scaling(
            frame_data,
            x_limits=x_limits,
            y_limits=y_limits,
            target_path=target_path / f"dimensional_scaling_comparison_{frame_number}.pdf",
        )
    frame_data = pd.concat([
        automatic_means.loc[automatic_means["s"] <= selectivities[-1]],
        va_means,
    ], ignore_index=True)
    _plot_sdss_scaling(
        frame_data,
        x_limits=x_limits,
        y_limits=y_limits,
        target_path=target_path / f"dimensional_scaling_comparison_{len(selectivities)+1}.pdf",
    )
...
if __name__ == "__main__":
    ...
    print_dimensional_scaling_io_statistics()
    create_incremental_volume_sequences()
    create_incremental_runtime_storage_sequence(only_averages=False)
    create_incremental_dimensional_scaling_sequence()
